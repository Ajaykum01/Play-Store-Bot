import os
import threading
import random
import string
import aiohttp
import urllib.parse
from http.server import BaseHTTPRequestHandler, HTTPServer
from pyrogram import Client, filters
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup
from pymongo import MongoClient
from dotenv import load_dotenv
from datetime import datetime

load_dotenv()

# ───────────────── MongoDB ─────────────────

MONGO_URL = os.getenv("MONGO_URL")
client = MongoClient(MONGO_URL)
db = client["telegram_bot"]

config_collection = db["config"]
users_collection = db["users"]
tokens_collection = db["tokens"]

ADMINS = [int(i) for i in os.getenv("ADMINS", "2117119246").split()]

# ───────────────── Bot ─────────────────

Bot = Client(
    "Play-Store-Bot",
    bot_token=os.environ["BOT_TOKEN"],
    api_id=int(os.environ["API_ID"]),
    api_hash=os.environ["API_HASH"]
)

HOW_TO_VERIFY_URL = "https://t.me/kpslinkteam/52"

FORCE_SUB_LINKS = [
    "https://telegram.me/+Iyc7cjYrBpxlOWM1",
    "https://telegram.me/+poyQjeODmb0wMjhl",
    "https://telegram.me/ffunusedaccountbot",
]

# ───────────────── Shortener APIs ─────────────────

AROLINKS_API = "7a04b0ba40696303483cd4be8541a1a8d831141f"
TVKURL_API = "9986767adc94f9d0a46a66fe436a9ba577c74f1f"

# ───────────────── Redeem Codes ─────────────────

def load_codes():
    config = config_collection.find_one({"_id": "codes"}) or {}
    return config.get("codes", [])

def save_codes(codes):
    config_collection.update_one(
        {"_id": "codes"},
        {"$set": {"codes": codes}},
        upsert=True
    )

def get_current_code():
    codes = load_codes()
    if not codes:
        return None
    code = codes.pop(0)
    save_codes(codes)
    return code

# ───────────────── Helpers ─────────────────

def gen_token(length=16):
    return ''.join(random.choices(string.ascii_letters + string.digits, k=length))

def ensure_user(user_id):
    if not users_collection.find_one({"_id": user_id}):
        users_collection.insert_one({"_id": user_id})

# ───────────────── Shorteners ─────────────────

async def shorten_with_arolinks(url: str) -> str:
    encoded = urllib.parse.quote_plus(url)
    api = f"https://arolinks.com/api?api={AROLINKS_API}&url={encoded}&format=text"

    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(api, timeout=20) as r:
                text = (await r.text()).strip()
                if text.startswith("http"):
                    return text
    except Exception:
        pass

    return url

async def shorten_with_tvkurl(url: str) -> str:
    encoded = urllib.parse.quote_plus(url)
    api = f"https://tvkurl.site/api?api={TVKURL_API}&url={encoded}&format=text"

    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(api, timeout=20) as r:
                text = (await r.text()).strip()
                if text.startswith("http"):
                    return text
    except Exception:
        pass

    return url

async def build_verify_link(bot: Client, token: str) -> str:
    me = await bot.get_me()
    deep_link = f"https://t.me/{me.username}?start=GL{token}"

    # Step 1 → AroLinks
    aro_link = await shorten_with_arolinks(deep_link)

    # Step 2 → TVKURL
    final_link = await shorten_with_tvkurl(aro_link)

    return final_link

# ───────────────── Handlers ─────────────────

@Bot.on_message(filters.command("start") & filters.private)
async def start(bot, message):
    user_id = message.from_user.id
    ensure_user(user_id)

    if len(message.command) > 1:
        payload = message.command[1]

        if payload.startswith("GL"):
            token = payload[2:]
            tok = tokens_collection.find_one({"_id": token})

            if not tok:
                return await message.reply("⚠️ Token expired or invalid.")

            if tok["user_id"] != user_id:
                return await message.reply("⚠️ This verification link is not yours.")

            if tok["used"]:
                return await message.reply("ℹ️ Already verified. Generate again.")

            return await message.reply(
                "✅ Short link completed!\n\nClick below to finish verification.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("Verify now by clicking me ✅", callback_data=f"final_verify:{token}")]
                ])
            )

    buttons = [[InlineKeyboardButton("Subscribe Channel 😎", url=i)] for i in FORCE_SUB_LINKS]
    buttons.append([InlineKeyboardButton("Verify ✅", callback_data="verify")])

    await message.reply(
        "JOIN GIVEN CHANNEL TO GET REDEEM CODE",
        reply_markup=InlineKeyboardMarkup(buttons)
    )

@Bot.on_callback_query(filters.regex("^verify$"))
async def verify(bot, query):
    await query.message.delete()
    await query.message.reply(
        "🙏 Welcome!\nClick Generate Code to start verification.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Generate Code", callback_data="gen_code")]
        ])
    )

@Bot.on_callback_query(filters.regex("^gen_code$"))
async def gen_code(bot, query):
    user_id = query.from_user.id
    ensure_user(user_id)

    token = gen_token()
    tokens_collection.insert_one({
        "_id": token,
        "user_id": user_id,
        "used": False,
        "created_at": datetime.utcnow()
    })

    verify_url = await build_verify_link(bot, token)

    await bot.send_message(
        user_id,
        "🔐 Verification Required\n\nClick Verify and complete shortlink.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Verify 🙂", url=verify_url)],
            [InlineKeyboardButton("How to verify ❓", url=HOW_TO_VERIFY_URL)]
        ]),
        disable_web_page_preview=True
    )

@Bot.on_callback_query(filters.regex("^final_verify:(.+)$"))
async def final_verify(bot, query):
    user_id = query.from_user.id
    token = query.data.split(":")[1]

    tok = tokens_collection.find_one({"_id": token})

    if not tok or tok["user_id"] != user_id or tok["used"]:
        return await query.answer("Invalid or expired token.", show_alert=True)

    tokens_collection.update_one(
        {"_id": token},
        {"$set": {"used": True, "used_at": datetime.utcnow()}}
    )

    code = get_current_code()

    text = (
        f"🎁 Redeem Code:\n\n`{code}`"
        if code else
        "❌ No redeem codes available right now."
    )

    await bot.send_message(
        user_id,
        f"✅ Verification Successful!\n\n{text}",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Generate Again", callback_data="gen_code")]
        ])
    )

# ───────────────── Admin ─────────────────

@Bot.on_message(filters.command("time") & filters.private)
async def set_codes(bot, message):
    if message.from_user.id not in ADMINS:
        return await message.reply("Unauthorized")

    codes = message.text.split()[1:]
    save_codes(codes)
    await message.reply(f"✅ {len(codes)} codes saved.")

# ───────────────── Health Check ─────────────────

class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        self.send_response(200)
        self.end_headers()
        self.wfile.write(b"Bot is Alive")

def run_server():
    HTTPServer(("0.0.0.0", 8080), HealthHandler).serve_forever()

# ───────────────── Main ─────────────────

if __name__ == "__main__":
    threading.Thread(target=run_server, daemon=True).start()
    Bot.run()
